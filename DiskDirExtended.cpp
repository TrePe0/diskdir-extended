...
INT_PTR CALLBACK DialogFunc2(HWND hdwnd, UINT Msg, WPARAM wParam, LPARAM lParam);
...
int ListCAB(const char* cabinet_fullpath)
{
	...
	typedef BOOL (DIAMONDAPI* fFdiCopy) (HFDI, char FAR*, char FAR*, int, PFNFDINOTIFY, PFNFDIDECRYPT, void FAR*);
	typedef BOOL (DIAMONDAPI* fFdiDestroy) (HFDI);
	static fFdiCreate fFDICreate = NULL;
	static fFdiCopy fFDICopy = NULL;
	static fFdiIsCabinet fFDIIsCabinet = NULL;
	static fFdiDestroy fFDIDestroy = NULL;
	HFDI			hfdi;
	ERF				erf;
	...
	if (hdll == NULL) {
		if(!(hdll = LoadLibrary("cabinet.dll"))) {
			return 0;
		}
		fFDICreate = (fFdiCreate) GetProcAddress(hdll, "FDICreate");
		fFDICopy = (fFdiCopy) GetProcAddress(hdll, "FDICopy");
		fFDIIsCabinet  = (fFdiIsCabinet)  GetProcAddress(hdll, "FDIIsCabinet");
		fFDIDestroy = (fFdiDestroy) GetProcAddress(hdll, "FDIDestroy");

		if (!fFDICreate || !fFDIIsCabinet || !fFDICopy || !fFDIDestroy) {
			hdll = NULL;
			return 0;
		}
	}
	...
	if (hf == -1)
	{
		(void) fFDIDestroy(hfdi);
		return 0;
	}

	if (fFDIIsCabinet(hfdi, hf, &fdici) == FALSE)
	{
...
INT_PTR CALLBACK DialogFunc(HWND hdwnd, UINT Msg, WPARAM wParam, LPARAM lParam);
...
INT_PTR CALLBACK DialogFunc(HWND hdwnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(Msg)
	{
		case WM_INITDIALOG:
			settings.readConfig();
			{ // add tooltip to donate button
				HWND hwndTip = CreateWindowEx(NULL, TOOLTIPS_CLASS, NULL,
					WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP,
					CW_USEDEFAULT, CW_USEDEFAULT,
					CW_USEDEFAULT, CW_USEDEFAULT,
					hdwnd, NULL, dllInstance,
					NULL);
				...
				toolInfo.uId = (UINT_PTR)GetDlgItem(hdwnd, IDC_DONATE);
				char res[8192];
				LoadString(dllInstance, IDS_DONATION, (LPSTR) res, 8192);
				toolInfo.lpszText = res;
...
*/
				...
				case IDC_DONATE:
					ShellExecute(NULL, "open", "https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=8B6YCYYVZAEDY&lc=US&item_name=DiskDirExtended&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted", NULL, NULL, SW_SHOWNORMAL);
					return TRUE;
